...
OUTPUT_PARQUET = PROCESSED_DIR / "alarms_clean.parquet"
...
def _clean(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    df["region"] = df["region_city"].astype(str).str.lower().str.strip().map(REGION_CITY_MAP).replace("__drop__", pd.NA)
    ...
    df["time_of_day"] = pd.cut(
        df["hour"],
        bins=[-1, 5, 11, 17, 24],
        labels=["Night (00-06)", "Morning (06-12)", "Day (12-18)", "Evening (18-24)"]
    )
    ...
    actual_dtypes = {k: v for k, v in CLEAN_DTYPES.items() if k in df.columns}
    df = df.astype(actual_dtypes)

    return df, issues

def _print_report(df_out: pd.DataFrame, n_raw: int, issues: list[str]) -> None:
    by_region = df_out.groupby("region", observed=False).size().sort_values(ascending=False)
    ...
    if "alarm_source" in df_out.columns:
        ...
        for src, cnt in df_out["alarm_source"].value_counts(dropna=False).items():
            print(f"{src}: {cnt:,}")
    if issues:
...
def process() -> None:
    ...
    print(f"\n{'=' * 65}")
    print(f"  COMPLETE -> {OUTPUT_PARQUET.name}  ({len(df_out):,} events)")
    print(f"{'=' * 65}")
...
def process_incremental() -> None:
    ...
    if not OUTPUT_PARQUET.exists():
        print("  No existing alarms_clean.parquet - run --process first.")
        return
    ...
    actual_dtypes = {k: v for k, v in CLEAN_DTYPES.items() if k in combined.columns}
    combined = combined.astype(actual_dtypes)
    ...
    print(f"Total:          {len(combined):,} events -> {OUTPUT_PARQUET.name}")
    print(f"{'=' * 65}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process alarms-merged.csv - alarms_clean.parquet",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--process", action="store_true",
                        help="Full reprocess of all data")
    parser.add_argument("--incremental", action="store_true",
                        help="Append only new rows to existing alarms_clean.parquet")
