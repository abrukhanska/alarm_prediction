...
COLLECTED_LOG = RAW_DIR / ".collected_dates"
...
def _load_collected_dates() -> set[str]:
    if not COLLECTED_LOG.exists():
        return set()
    lines = COLLECTED_LOG.read_text(encoding="utf-8").splitlines()
    return {ln.strip() for ln in lines if ln.strip()}


def _mark_date_collected(d: date) -> None:
...
def fetch_region_alarms(uid: int, region: str, logger: logging.Logger) -> list[dict] | None:
    if not API_KEY:
...
def alarms_for_date(alarms: list[dict], target_date: date, region: str) -> list[dict]:
    parsed_alarms = []
...
def write_alarms(region: str, day_alarms: list[dict], logger: logging.Logger) -> int:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pull alarms from alerts.in.ua history API",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--date", type=str, default=None,
        help="Конкретна дата YYYY-MM-DD (default: вчора)",
    )
    parser.add_argument(
        "--days", type=int, default=1,
        help="Зібрати останні N днів (default: 1 = вчора)",
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Зібрати навіть якщо дата вже є в .collected_dates",
    )
    ...
    collected = _load_collected_dates()
    if not args.force:
        skipped = [d for d in target_dates if str(d) in collected]
        target_dates = [d for d in target_dates if str(d) not in collected]
        if skipped:
            logger.info(f"Skipping already collected dates: {skipped}")
            logger.info("Use --force to re-collect them.")

    if not target_dates:
        logger.info("Nothing to collect - all dates already done.")
    ...
    logger.info(f"Output: {OUTPUT_CSV}  [APPEND mode - data is never deleted here]")
    logger.info("=" * 60)
    ...
    for d in target_dates:
        ...
        if stats["errors"] == 0:
            _mark_date_collected(d)
        else:
            logger.warning(
                f"Date {d} had {stats['errors']} errors - NOT marked as collected, "
